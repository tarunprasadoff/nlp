...
for epoch in range(num_epochs):
    ...
    for testload in enumerate(tqdm(testloader)):
        ...
        y_pred = skipGram.forward(testload[1][:,0].to(device))
    ...
    torch.save(skipGram.state_dict, f"Results/SkipGram/Gpu/{str(learning_rate)[2:]}/{epoch+1}.pth")
    results.to_csv(f"Results/SkipGram/Gpu/{str(learning_rate)[2:]}/Loss.csv")